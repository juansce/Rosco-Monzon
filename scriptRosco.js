...
// recupero los valores del formulario que estan en la url (jugador)
let jugadores = new Jugante((new URLSearchParams(window.location.search).get("jugador")),0)

showPlayer.innerHTML = `Hola ${(jugadores.nombre).toUpperCase()}, vamos a divernirnos!!!`
...
botResp.addEventListener("click",() => {
    ...
    puntos.innerHTML = `Tenes ganados ${jugadores.puntos} puntos`
})